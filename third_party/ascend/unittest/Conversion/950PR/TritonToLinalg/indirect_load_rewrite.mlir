...
// -----
// V1 rank-1 disabled (AddPtr, static power-of-two stride 4, masked single tile):
// Keep this on the normal strided DMA path instead of routing to indirect_load.
...
// -----
// V1 rank-1 disabled (AddPtr, static non-power-of-two stride 3):
// tt.addptr(splat ptr, arange*3) should stay on the normal strided DMA path.
...
// -----
// V1 disabled (make_tensor_ptr, non-permuted, low-dim non-power-of-two stride 5):
// strides=[256, 5], order=[1,0] should stay on the normal strided DMA path.
...
// -----
// V1 rank-1 disabled (make_tensor_ptr, static non-power-of-two stride 7):
...
// -----
// V2 disabled (AddPtr Store, 1D, static non-power-of-two stride 3):
// tt.store(tt.addptr(splat ptr, arange*3), value) should stay on DMA lowering.
...
// -----
// V2 disabled (make_tensor_ptr Store, non-permuted, low-dim non-power-of-two stride 5):
...
// -----
// V1.5 disabled (make_tensor_ptr Load + boundary_check + PAD_ZERO, low-dim non-power-of-two stride 5):
// Block 8x8 covers a 7x5 parent; keep it off tt.indirect_load.
...
// -----
// V1.5 disabled (make_tensor_ptr Load + boundary_check + PAD_NAN, low-dim non-power-of-two stride 5):
...
// -----
// V1.5 disabled (make_tensor_ptr Load + boundary_check + SCALAR-AddPtr base):
...
// -----
// V1.5 disabled (make_tensor_ptr Store + boundary_check, low-dim non-power-of-two stride 5):
